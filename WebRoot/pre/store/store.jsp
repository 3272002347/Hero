...
<body>
				...
				<div class="mainlist">
					<ul>
						...
						<a href="<%=path%>/HeroStoreServlet">
							<li style="
	background-color: #3C404B;">英雄</li>
						</a>
						<a href="<%=path%>/SkinStoreServlet"><li>皮肤</li>
						</a>
					</ul>
				</div>
				...
				<div class="viewlist">
					<div>
						<span class="remind">上架商品:英雄</span>
					</div>
					<div id="viewDiv"
						style="height:1200px;width:914px;display:inline-block;">
						<c:forEach var="item" items="${items.resultList }"
							varStatus="status">
							<dl>
								<dt>
									<a
										href="<%=path %>/HeroDefailServlet?id=${item.id}"
										target="_blank"> <img src="${item.imgsrc }" width="122"
										height="150"> </a>
								...
								<dd style="text-align:center;">
									<a
										href="<%=path %>/HeroDefailServlet?id=${item.id}"
										class="djname" target="_blank"
										title="冬${item.heroname
											}大礼包"> <strong>${item.heroname
											}</strong> </a>
									<div class="bxlist-t1">
									...
									<div class="bxlist-t2 c">
										<a href="<%=path %>/HeroDefailServlet?id=${item.id}" class="btn-green buy-now" title="立即购买">立即购买</a>
									</div>
								</dd>
							</dl>
						</c:forEach>
	...
	<script type="text/javascript">
		...
		function getpage(pageNo){
			$.post("/HeroWeb/GetPageStoreServlet",{"pageNo":pageNo},function(data){
				...
				$.each(resultlist,function(index,value){
					...
					+"<a href='/lol/item/14771.shtml?ADTAG=innercop.lol.SY.recommend_new'"
						+"class='djname' target='_blank' title='"+value.heroname+"礼包'> <strong>"+value.heroname+""
